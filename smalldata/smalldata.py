...
def _allocate_group(servers, clients):
    if isinstance(servers, MPI.Group) and isinstance(clients, MPI.Group):
        server_group = servers
        client_group = clients
    elif (type(servers) is int) and (type(clients) is int):
        world_group = COMM.Get_group()
        server_group = world_group.Incl(range(servers))
        client_group = world_group.Incl(range(servers, clients))
    return server_group, client_group
...
class SmallData:
    ...
    def __init__(self, servers=None, clients=None, 
                 filename=None, batch_size=10000, cache_size=None,
                 callbacks=[]):
        """
        Parameters
        ----------
        server_group : int, MPI.Group
            The MPI group to allocate to server processes, or number
            of cores to make into a group

        client_group : int, MPI.Group
            The MPI group to allocate to client processes, or number
            of cores to make into a group
        ...
        if MODE == 'PARALLEL':
            ...
            server_group, client_group = _allocate_group(servers, clients)
